...
var Job = Ember.Object.extend({ // Instance methods
    ...
    updateState: function(state) {
        ...
        return Job._request({
            method: 'PUT',
            url: `${config.baseURL}/job/${id}/state/${state}`
        });
    },
...
Job.reopenClass({ // Class methods
    ...
    find: function(opts={}) {
        ...
        var url = `${config.baseURL}/${from}..${to}`;

        if(opts.type && opts.state) {
            url = `${config.baseURL}/jobs/${opts.type}/${opts.state}/${from}..${to}`;
        } else if(opts.type) {
            url = `${config.baseURL}/jobs/${opts.type}/${from}..${to}`;
        } else if(opts.state) {
            url = `${config.baseURL}/jobs/${opts.state}/${from}..${to}`;
        }
    ...
    findOne: function(opts={}) {
        return this._request({
            method: 'GET',
            url: `${config.baseURL}/job/${opts.id}`
        });
    },
    ...
    stats: function(opts={}) {
        ...
        if (!Ember.empty(type) && !Ember.empty(state)) {
            url = `${config.baseURL}/jobs/${type}/${state}/stats`;
        } else {
